      ...
      <el-table-column
        ...
        <template scope="scope">
          ...
          <el-button
            ...
            v-html="scope.row.state==='可用' ? '下架' : '发布'"></el-button>
          <el-button
            size="small"
            class="edit-btn"
...
export default {
  ...
  data() {
    return {
      ...
      url: "/api/recruitment/recruitments",
      filterTmpl: {
        publishUser: {
          label: "发布者",
          inputType: 0 // 0 代表 input
  ...
  methods: {
    ...
    handleMore(index, row) {
      this.$router.push(`/check/recruitmentInfo/${row.id}`);
    },
    handlePublic(index, row) {
      let state = row.state === "可用" ? "不可用" : "可用";
      let recruitment = {
        id: row.id,
        state: state
      };
      axios
        .post("/api/recruitment/change/recruitment", { recruitment })
        .then(res => {
    ...
    handleEdit(index, row) {
      this.$router.push(`/edit/recruitmentInfo/${row.id}`);
    },
    //        单页大小改变回调事件
