      ...
      <el-table-column
        ...
        <template scope="scope">
          ...
          <el-button
            size="small"
            class="edit-btn"
            @click="nextLevel(scope.$index, scope.row)">下一阶段
          </el-button>
           <el-button
            size="small"
            class="edit-btn"
          ...
          <el-button
            size="small"
            type="danger"
            @click="handleDelete(scope.$index, scope.row)">{{ scope.row.status !== '不可用' ? '禁用' : '启用'}}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    handleMore(index, row) {
      console.log(row);
      this.$router.push(`/check/pendProjectInfo/${row.id}`);
    },
    //        删除按钮事件
    handleDelete(index, row) {
      let state = row.status !== "不可用" ? "不可用" : this.statusOptions[0];
      console.log(row);
     this.changeStatus(row, state)
    },
    changeStatus(row, status) {
      console.log(row, status)
      row.status = status
      axios.post('/api/project/change/pendProject', {
        id: row.id,
        info: {status: status}
      })
      .then(res => {
    ...
    nextLevel(index, row) {
      let _index = this.statusOptions.indexOf(row.status);
      console.log(_index)
      if (_index == 3 || _index == -1) return;
      let status = this.statusOptions[_index + 1];
      this.changeStatus(row, status)
    },
    lastLevel(index, row) {
      let _index = this.statusOptions.indexOf(row.status);
      if (_index == 0 || _index == -1) return;
      let status = this.statusOptions[_index - 1];
      this.changeStatus(row, status)
    },
    //        编辑按钮事件
    handleEdit(index, row) {
      this.$router.push(`/edit/pendProjectInfo/${row.id}`);
    },
    //        单页大小改变回调事件
